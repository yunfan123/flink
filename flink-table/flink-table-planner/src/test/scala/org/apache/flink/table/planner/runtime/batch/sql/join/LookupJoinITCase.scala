...
class LookupJoinITCase extends BatchTestBase {
  ...
  def testLeftJoinTemporalTableWithLocalPredicate(): Unit = {
    val sql =
      s"SELECT T.id, T.len, T.content, D.name, D.age FROM T LEFT JOIN userTable$partitionJoinHint " +
  ...
  def testJoinTemporalTable(): Unit = {
    val sql = s"SELECT T.id, T.len, T.content, D.name FROM T JOIN userTable$partitionJoinHint " +
      "for system_time as of T.proctime AS D ON T.id = D.id"
  ...
  def testJoinTemporalTableWithNonEqualFilter(): Unit = {
    val sql =
      s"SELECT T.id, T.len, T.content, D.name, D.age FROM T JOIN userTable$partitionJoinHint " +
  ...
  def testJoinTemporalTableOnMultiFields(): Unit = {
    val sql = s"SELECT T.id, T.len, D.name FROM T JOIN userTable$partitionJoinHint " +
  ...
  def testLeftJoinTemporalTable(): Unit = {
    val sql = s"SELECT T.id, T.len, D.name, D.age FROM T LEFT JOIN userTable$partitionJoinHint " +
      "for system_time as of T.proctime AS D ON T.id = D.id"
  ...
  def testJoinTemporalTableOnMultiKeyFieldsWithNullData(): Unit = {
    val sql =
      s"SELECT T.id, T.len, D.name FROM nullableT T JOIN userTableWithNull$partitionJoinHint " +
        "for system_time as of T.proctime AS D ON T.content = D.name AND T.id = D.id"
  ...
  def testLeftJoinTemporalTableOnMultiKeyFieldsWithNullData(): Unit = {
    val sql =
      s"SELECT D.id, T.len, D.name FROM nullableT T LEFT JOIN userTableWithNull$partitionJoinHint " +
        "for system_time as of T.proctime AS D ON T.content = D.name AND T.id = D.id"
    val expected = Seq(
  ...
  def testJoinTemporalTableOnNullConstantKey(): Unit = {
    val sql = s"SELECT T.id, T.len, T.content FROM T JOIN userTable$partitionJoinHint " +
  ...
  def testJoinTemporalTableWithComputedColumn(): Unit = {
    ...
    val sql = s"SELECT T.id, T.len, T.content, D.name, D.age, D.nominal_age " +
      s"FROM T JOIN userTableWithComputedColumn$partitionJoinHint " +
      "for system_time as of T.proctime AS D ON T.id = D.id"
    ...
    try {
      ...
      val sql =
        s"""
           ...
           |LEFT JOIN userTable$partitionJoinHint FOR SYSTEM_TIME AS OF T.proc AS D 
           |ON T.id = D.id
           |""".stripMargin
