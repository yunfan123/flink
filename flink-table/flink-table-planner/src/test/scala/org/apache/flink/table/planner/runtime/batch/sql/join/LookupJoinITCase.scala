...
class LookupJoinITCase extends BatchTestBase {
  ...
  def testJoinTemporalTable(): Unit = {
    val sql = s"SELECT$tableDHashShuffleHint T.id, T.len, T.content, D.name FROM T JOIN userTable " +
      "for system_time as of T.proctime AS D ON T.id = D.id"
  ...
  def testLeftJoinTemporalTable(): Unit = {
    val sql = s"SELECT$tableDHashShuffleHint T.id, T.len, D.name, D.age FROM T LEFT JOIN userTable " +
      "for system_time as of T.proctime AS D ON T.id = D.id"
  ...
  def testJoinTemporalTableWithComputedColumn(): Unit = {
    ...
    val sql = s"SELECT$tableDHashShuffleHint T.id, T.len, T.content, D.name, D.age, D.nominal_age " +
