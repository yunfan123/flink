...
class LookupJoinITCase(
  ...
  def testJoinTemporalTableWithUdfEqualFilter(): Unit = {
    val sql =
      s"""
         ...
         |FROM
         |  src AS T JOIN user_table$partitionJoinHint for system_time as of T.proctime AS D
         |ON T.id = D.id
  ...
  def testJoinTemporalTableOnNullableKey(): Unit = {
    val sql =
      s"SELECT T.id, T.len, D.name FROM nullable_src AS T JOIN user_table$partitionJoinHint " +
        "for system_time as of T.proctime AS D ON T.id = D.id"
  ...
  def testJoinTemporalTableOnMultiKeyFields2(): Unit = {
    ...
    val sql = "SELECT t1.id, t1.len, D.name FROM " +
      ...
      s"JOIN user_table$partitionJoinHint for system_time as of t1.proctime AS D " +
  ...
  def testLeftJoinTemporalTableWithUdfPreFilter(): Unit = {
    ...
    val sql =
      s"SELECT T.id, T.len, T.content, D.name FROM src AS T LEFT JOIN user_table$partitionJoinHint " +
  ...
  def testLeftJoinTemporalTableOnNullableKey(): Unit = {
    val sql =
      s"SELECT T.id, T.len, D.name FROM nullable_src AS T LEFT OUTER JOIN user_table$partitionJoinHint " +
        "for system_time as of T.proctime AS D ON T.id = D.id"
  ...
  def testJoinTemporalTableOnMultiKeyFieldsWithNullData(): Unit = {
    val sql =
      s"SELECT T.id, T.len, D.name FROM nullable_src AS T JOIN nullable_user_table$partitionJoinHint " +
        "for system_time as of T.proctime AS D ON T.content = D.name AND T.id = D.id"
  ...
  def testLeftJoinTemporalTableOnMultiKeyFieldsWithNullData(): Unit = {
    val sql =
      s"SELECT D.id, T.len, D.name FROM nullable_src AS T LEFT JOIN nullable_user_table$partitionJoinHint " +
        "for system_time as of T.proctime AS D ON T.content = D.name AND T.id = D.id"
  ...
  def testJoinTemporalTableOnNullConstantKey(): Unit = {
    val sql =
      s"SELECT T.id, T.len, T.content FROM nullable_src AS T JOIN nullable_user_table$partitionJoinHint " +
  ...
  def testJoinTemporalTableOnMultiKeyFieldsWithUDF(): Unit = {
    val sql =
      s"SELECT T.id, T.content, D.age, D.id FROM src AS T JOIN user_table$partitionJoinHint " +
  ...
  def testJoinTemporalTableWithComputedColumn(): Unit = {
    ...
    val sql = s"SELECT T.id, T.len, T.content, D.name, D.age, D.nominal_age " +
      s"FROM src AS T JOIN userTableWithComputedColumn$partitionJoinHint " +
      "for system_time as of T.proctime AS D ON T.id = D.id"
  ...
  def testAggAndJoinAllConstantKeyWithTryResolveMode(): Unit = {
    ...
    val sql2 = s"SELECT t1.id FROM t1 LEFT JOIN user_table$partitionJoinHint " +
      "for system_time as of t1.proctime AS D ON D.id = 3"
